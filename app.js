...
function renderHome() {
    const _set = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
    _set('heroActive', activeSkills.length);
    _set('heroPassive', passiveSkills.length);
    _set('heroTotal', activeSkills.length + passiveSkills.length);
    _set('heroAffix', affixes.length);
    _set('heroEquipment', equipmentData.length);
    _set('heroAttr', attributes.length);
...
function renderGems(filteredData) {
    ...
    grid.innerHTML = data.map(gem => {
        ...
        return `
            <div class="equipment-card" data-gem-id="${gem.id}" onclick="openGemDetail('${gem.id}')" style="border-left-color:#9b59b6">
                <div class="equipment-card-header">
                    <span class="equipment-card-icon">💎</span>
                    <div>
